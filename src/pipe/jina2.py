...
# Disable tqdm in the Jina model
...
class JinaV4Pipeline:
    """
    Unified multimodal retrieval pipeline for Jina Embeddings v4.
    
    Jina v4 uses a single-stream architecture where text and images
    are processed through a shared transformer pathway, creating 
    ...
    def __init__(self, 
        ...
        """
        Initialize the Jina v4 pipeline.
        ...
        """
        print(f"Loading Jina v4 model on {Config.DEVICE}...")
    ...
    def encode_images(self, 
        ...
        with torch.no_grad(), torch.amp.autocast(device_type=Config.DEVICE, enabled=(Config.DEVICE == "cuda")):
            for i in range(0, len(images), batch_size):
                batch = images[i:i + batch_size]
                
                # Use the model's encode_image method
                # Note: encode_image doesn't support prompt_name parameter
    ...
    def encode_texts(self, 
        ...
        with torch.no_grad(), torch.amp.autocast(device_type=Config.DEVICE, enabled=(Config.DEVICE == "cuda")):
            for i in range(0, len(texts), batch_size):
                ...
                # Build kwargs - check if prompt_name is supported
                ...
                try:
                    ...
                    # If prompt_name not supported, try without it
                    embeddings = self.model.encode_text(
                        texts=batch,
                        task=self.task,
                        show_progress_bar=False
...
def run_evaluation(datasets: List[str], 
    ...
    for ds_name in datasets:
        ...
        log_file = f"logs/jina_v4/{ds_name}_{timestamp}.log"
...
def main():
    ...
    datasets = [ "MSCOCO_i2t"]
    perturbations = ["clean"] #, "flip", "gauss1", "bright", "grayscale", "gauss2", "compress"]
